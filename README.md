...
- 首次、UIDVALIDITY 变化、游标闲置超过 15 分钟时，只补查末尾最多 128 条头部，正文限最近 15 分钟；正常活跃期间从持久化主号 UID 游标增量处理。收到请求且观察已超过 5 秒时共享补查，不依赖 Apple 每次都及时发送 IDLE 通知；无人请求不触发周期补查，错误退避重试。public IMAPS 仅读本地归档。详见 [收信与资源设计](docs/MAIL-RECEPTION.md)。
