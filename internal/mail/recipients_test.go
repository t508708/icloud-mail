...
func TestDirectICloudTargetFallbackRequiresOneExactTargetRecipient(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			name:   "same domain tag returns registered root",
			header: stdmail.Header{"To": {"Hide My Email <root+job-42@icloud.com>"}},
			target: 1, matched: true,
		},
		{
			name:   "apple normalized tag returns registered root",
			header: stdmail.Header{"To": {"Hide My Email <root@icloud.com>"}},
			target: 1, matched: true,
		},
		{
			name:   "full registered address wins over root",
			header: stdmail.Header{"To": {"root+literal@icloud.com"}},
			target: 2, matched: true,
		},
		{
			name:   "unknown root does not match",
			header: stdmail.Header{"To": {"unknown+job-42@icloud.com"}},
			target: 1,
		},
		{
			name:   "different domain does not match",
			header: stdmail.Header{"To": {"root+job-42@other.example"}},
			target: 1,
		},
		{
			name:   "multiple recipient fields do not match",
			header: stdmail.Header{"To": {"root@icloud.com"}, "Cc": {"other@example.com"}},
			target: 1,
		},
		{
			name:   "strong delivery metadata remains authoritative",
			header: stdmail.Header{"To": {"root@icloud.com"}, "Delivered-To": {"primary@icloud.com"}},
			target: 1,
		},
	} {
		t.Run(test.name, func(t *testing.T) {
