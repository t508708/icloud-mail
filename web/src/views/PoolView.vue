      ...
      <el-tabs v-model="tab" @tab-change="refresh">
        ...
        <el-tab-pane label="库存与定时创建" name="accounts">
          <p class="pool-hint">
            ...
            0
            时按计划创建；达到正数目标时跳过创建，领取后自动恢复。手动批量创建和单次探测均不使用本地创建额度，成功后不额外等待；仍受 Apple 实际响应影响。自动计划按滚动 1 小时最多 18 次、至少间隔 2 分钟执行。自动只选择初始通道，不因限流切换；Apple 明确限流后暂停至少 24 小时。本地上限不代表 Apple 官方配额保证。
          </p>
          <el-table
            :data="accounts"
