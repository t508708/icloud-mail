      ...
      <el-tabs v-model="tab" @tab-change="refresh">
        ...
        <el-tab-pane label="库存与定时创建" name="accounts">
          <p class="pool-hint">
            ...
            0
            时按计划创建；达到正数目标时跳过创建，领取后自动恢复。后台批量与自动创建共享本地预算：滚动 1 小时最多 25 次，至少间隔 2 分钟；手动探测使用独立同等额度，不消耗后台额度，也不受后台 Apple 24 小时冷却阻挡，且不修改后台冷却或计划。自动只选择初始通道，不因限流切换；Apple 明确限流后暂停至少 24 小时。本地上限不代表 Apple 官方配额保证。
          </p>
          <el-table
            :data="accounts"
