...
从 [v0.2.1 Release](https://github.com/t508708/icloud-mail/releases/tag/v0.2.1) 下载 `icloud-mail-v0.2.1-linux-amd64.zip` 和 `SHA256SUMS`；仅适用于 Linux x86_64 / amd64。完整复制命令见 [README](README.md#docker-部署)。
...
校验通过后再解压和导入。包内包含 `icloud-api:v0.2.1` 与 `icloud-api-postgres:v0.2.1`，无需本机编译或拉取基础镜像。`RELEASE.json` 记录源码提交、架构和镜像 ID；包内 `SHA256SUMS` 可再次校验各文件。应用运行仍需访问 Apple/上游 IMAP。
...
历史 `handoff-2026.09.14` 包继续使用其自带的编排和镜像；本发布中的 `compose.offline.yaml` 指向 v0.2.1，不混用不同版本的离线文件。
