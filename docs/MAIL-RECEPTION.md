...
2. OTP、浏览器取件链接、旧邮件 API 和邮箱池取码按同一 alias 共用限流：请求处理中及完成后 2 秒内，后续请求直接返回 `429 RATE_LIMITED` 和 `Retry-After: 2`，成功与失败均计入。被拒绝请求不会延后这个冷却窗口。同步层同样只在完成后 2 秒内复用结果；同主号 IMAP 获取起点也维持最多每 2 秒一次，避免刚到达的验证码被本地冷却延迟。
