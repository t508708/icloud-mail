...
2. 共享模式下同一 root 的所有取件凭据共用容量 2 的令牌桶，每 2 秒恢复 1 次额度，最多积累 2 次。额度从请求准入时扣除，等待过程中持续恢复，结束、失败或取消不追加完成后冷却；被拦截请求不改恢复时刻。同 root 最多 1 个普通读取和 1 个 Pool 长等待，普通读取不被长等待独占。主号实际 IMAP 获取起点仍最短间隔 2 秒，由共享 receiver 合并，HTTP 突发额度不等于额外 Apple 建连或读取。
3. 活跃 receiver 最多服务 128 个主号；HTTP 取件最多 128 个在途请求、每主号 64 个，每秒最多接纳 100 次。超量直接返回 429，不排队。响应保留 `RATE_LIMITED`，用 `X-RateLimit-Reason` 区分速率、重复在途、等待槽和主号/全局容量；`Retry-After` 按剩余时间向上取整，`X-Retry-After-Ms` 提供毫秒值。在途完成时刻未知时提示 1 秒后复查，不承诺到时请求已结束。`IMAP_IDLE_ENABLED=false` 的旧路径保持全站 16、主号 2、同 alias 独占和完成后 2 秒冷却。后台与健康检查不占这些名额。
...
取件预算的时序、错误原因和客户端建议见 [分层取件限流](PICKUP-RATE.md)。
