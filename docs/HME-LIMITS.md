...
- 自动创建计划按主号使用本地预算：滚动 1 小时最多 18 次，最小间隔 2 分钟。手动批量创建和单次探测不使用本地额度，成功项之间不额外等待，也不修改自动计划或其 Apple 冷却。自动计划失败及待确认尝试计入额度；没有额外的每日 20 次限制。自动计划后端日额度为 432（18*24），作为 18 次/小时的冗余约束。该策略是本项目的保守上限，不是 Apple 官方额度承诺。
- Apple 明确返回限流后，该主号自动创建计划和手动批量任务暂停至少 24 小时；不自动从 Apple Account 切换到 iCloud Web，也不反向切换。`auto` 只选择本轮初始通道。单次手动探测仅返回本次 Apple 结果及其原始 Retry-After，不沿用后台的 24 小时等待。
