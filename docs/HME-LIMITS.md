...
- 定时及后台批量创建按主号共享本地预算：滚动 1 小时最多 18 次，最小间隔 2 分钟；手动单次探测使用独立 25 次/小时额度，不消耗后台额度，也不受后台 Apple 24 小时冷却阻挡，且不修改后台冷却或计划。失败及待确认尝试都占对应额度；没有额外的每日 20 次限制。后端日额度为 432（18*24），作为 18 次/小时的冗余约束。该策略是本项目的保守上限，不是 Apple 官方额度承诺。
- Apple 明确返回限流后，该主号后台创建暂停至少 24 小时；不自动从 Apple Account 切换到 iCloud Web，也不反向切换。`auto` 只选择本轮初始通道。手动探测仅返回本次 Apple 结果及其原始 Retry-After，不沿用后台的 24 小时等待。
...
- 额度按一次创建尝试递增，失败或结果待确认也计数；页面成功数则只统计已确认创建事件，分为滚动 1 小时和服务时区今日零点至当前，包含手动、批量和自动，排除目录导入；滚动预算不承诺固定完成时长。
