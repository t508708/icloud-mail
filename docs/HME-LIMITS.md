...
- 定时及后台批量创建按主号共享本地预算：滚动 1 小时最多 25 次，最小间隔 2 分钟；手动单次探测使用独立同等额度，不消耗后台额度，也不受后台 Apple 24 小时冷却阻挡，且不修改后台冷却或计划。失败及待确认尝试都占对应额度；没有额外的每日 20 次限制。后端日额度为 600，作为 25 次/小时的冗余约束。该策略是本项目的保守上限，不是 Apple 官方额度承诺。
