...
任务遇到限流至少等待 61 分钟，总时限 24 小时；每 4 分钟尝试管理 session 保活。停止只阻止后续请求，进行中的请求完成并保存后停止。应用重启后的 `interrupted` 任务不会自动重发。新通道已实现，但真实账号登录和每小时 25+ 吞吐尚未实测，不构成 Apple 配额保证。
...
- 管理新通道当前仍待用户独立登录/2FA；没有将模拟 30 个创建结果计作真实 Apple 吞吐。
