...
3. 开启定时创建。每主号每小时安排 40 次尝试，最短间隔 60 秒、平均约 90 秒；优先 Apple Account，明确限流后切换 iCloud Web，两者都限流时冷却 61 分钟。最多 3 个主号并行，同主号串行确认；实际创建量以 Apple 返回为准。需要一轮几十个时使用后台批量任务。
...
升级至每主号 40 次/小时后，已开启的旧计划会一次性更新，无需重新开关。历史记录和已有冷却保留；关闭的计划保持关闭，后续重启不会重置排期。
...
- 手动创建跳过本项目每主号每小时 40 次、最短 60 秒且平均约 90 秒的自动调度，也不依赖自动开关或库存目标。
